...
class SRModel(BaseModel):
    ...
    def test_crop9(self):
        ...
        if hasattr(self, "net_g_ema"):
            self.net_g_ema.eval()
            with torch.no_grad():
                imTL = self.net_g_ema(self.lq[:, :, 0 : h + rf, 0 : w + rf])[
                    :, :, 0:h, 0:w
                ]
            ...
            with torch.no_grad():
                ...
                imML = self.net_g(self.lq[:, :, h - rf : 2 * h + rf, 0 : w + rf])[
                    :, :, rf : (rf + h), 0:w
                ]
                imBL = self.net_g(self.lq[:, :, 2 * h - rf :, 0 : w + rf])[
                    :, :, rf:, 0:w
                ]
                imTM = self.net_g(self.lq[:, :, 0 : h + rf, w - rf : 2 * w + rf])[
                    :, :, 0:h, rf : (rf + w)
                ]
                imMM = self.net_g(
                    self.lq[:, :, h - rf : 2 * h + rf, w - rf : 2 * w + rf]
                )[:, :, rf : (rf + h), rf : (rf + w)]
                imBM = self.net_g(self.lq[:, :, 2 * h - rf :, w - rf : 2 * w + rf])[
                    :, :, rf:, rf : (rf + w)
                ]
                imTR = self.net_g(self.lq[:, :, 0 : h + rf, 2 * w - rf :])[
                    :, :, 0:h, rf:
                ]
                imMR = self.net_g(self.lq[:, :, h - rf : 2 * h + rf, 2 * w - rf :])[
                    :, :, rf : (rf + h), rf:
                ]
                imBR = self.net_g(self.lq[:, :, 2 * h - rf :, 2 * w - rf :])[
                    :, :, rf:, rf:
                ]
