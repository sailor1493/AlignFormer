...
class ECFNetModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(ECFNetModel, self).__init__(opt)
    ...
    def interpolate_down(self, in_ts):
        gt_2 = F.interpolate(in_ts, scale_factor=0.5)  # 1, 4, 128, 128
        gt_4 = F.interpolate(gt_2, scale_factor=0.5)  # 1, 4, 64, 64
        gt_8 = F.interpolate(gt_4, scale_factor=0.5)  # 1, 4, 32, 32
        gts = [gt_8, gt_4, gt_2, in_ts]
        return gts
    ...
    def optimize_parameters(self, current_iter):
        ...
        gts = self.interpolate_down(self.gt)

        if hasattr(self, "mask"):
            weights = self.interpolate_down(self.mask)
        else:
