...
class UDCUNet(nn.Module):
    def __init__(
        ...
        # Condition
        basic_Res = functools.partial(arch_util.ResidualBlockNoBN, num_feat=nf)

        self.cond_head = nn.Sequential(
            nn.Conv2d(in_nc, nf, 3, 1, 1), nn.LeakyReLU(0.2, True)
        )
        self.cond_first = arch_util.make_layer(basic_Res, 2)
        ...
        self.k_first = arch_util.make_layer(basic_Res, 2)
