...
class DISCNet(nn.Module):
    def __init__(
        ...
        self.conv_21 = conv_block(
            nf, 2 * nf, stride=2, kernel_size=kernel_size, act_type=act_type
        )
        ...
        self.conv_31 = conv_block(
            2 * nf, 4 * nf, stride=2, kernel_size=kernel_size, act_type=act_type
        )
        ...
        self.final_conv = conv_block(
            nf, out_nc, kernel_size=kernel_size, act_type=act_type
        )
        ...
        if self.kernel_cond:
            ...
            self.kconv_11 = conv_block(
                cond_nc, nf, kernel_size=kernel_size, act_type=act_type
            )
...
class ResBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            num_feat, num_feat, 3, 1, bias=True, padding=padding, dilation=dilation
        )
