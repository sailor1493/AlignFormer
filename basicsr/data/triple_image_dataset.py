...
class TripleImageDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.gt_folder, self.lq_folder, self.ref_folder = (
            opt["dataroot_gt"],
            opt["dataroot_lq"],
            opt["dataroot_ref"],
        )
        ...
        if self.io_backend_opt["type"] == "lmdb":
            ...
            self.paths = multiple_paths_from_meta_info_file(
                [self.lq_folder, self.gt_folder, self.ref_folder],
                ["lq", "gt", "ref"],
    ...
    def __getitem__(self, index):
        ...
        if self.opt["phase"] == "train":
            ...
            # random crop
            img_gt, img_lq, img_ref = multiple_random_crop(
                img_gt, img_lq, img_ref, gt_size, scale, gt_path
            )

            # flip, rotation
            img_gt, img_lq, img_ref = augment(
                [img_gt, img_lq, img_ref], self.opt["use_flip"], self.opt["use_rot"]
            )

        # TODO: color space transform
        # BGR to RGB, HWC to CHW, numpy to tensor
        img_gt, img_lq, img_ref = img2tensor(
            [img_gt, img_lq, img_ref], bgr2rgb=True, float32=True
        )
        # normalize
